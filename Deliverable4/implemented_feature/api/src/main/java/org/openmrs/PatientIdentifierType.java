...
public class PatientIdentifierType extends BaseChangeableOpenmrsMetadata {
	...
	/**
	 * Enumerates the possible ways that location may be applicable for a particular
	 * Patient Identifer Type
	 */
	...
	/**
	 * Enumeration for the way to handle uniqueness among identifiers for a given
	 * identifier type
	 */
	public enum UniquenessBehavior {
		...
		/**
		 * Indicates that identifiers should be unique only across a location if the
		 * identifier's location property is not null
		 */
		LOCATION
	}
	...
	/**
	 * the function checks the similarity of the current IdentifierType and the
	 * given IdentifierType if two consider the same then the function return true,
	 * otherwise return false
	 * 
	 * @param patientIdentifier
	 * @return the IdentifierType is equal to the given one
	 */
	public boolean isDuplicate(PatientIdentifierType patientIdentifier) {
		// if the id or the format is same, then two PatientIdentifierTypes are same
		if ((this.patientIdentifierTypeId == patientIdentifier.getPatientIdentifierTypeId())
				|| (this.format == patientIdentifier.getFormat())) {
			return true;
		} else if (this.formatDescription != null
				&& (this.formatDescription == patientIdentifier.getFormatDescription())) {
			// if the formatDescription is same, then two PatientIdentifierTypes are same
			return true;
		} else {
			// else the two given PatientIdentifierTypes are not same then return false
			return false;
		}
	}

	public void merge(PatientIdentifierType newType) {
		// peasant short circut
		if (format == null)
			format = newType.getFormat();
		required = required || newType.getRequired();
		if (formatDescription == null)
			formatDescription = newType.getFormatDescription();
		if (validator == null)
			validator = newType.getValidator();
		if (locationBehavior == null)
			locationBehavior = newType.getLocationBehavior();
		if (uniquenessBehavior == null)
			uniquenessBehavior = newType.getUniquenessBehavior();
	}
